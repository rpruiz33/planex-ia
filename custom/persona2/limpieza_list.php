...
/**
 *   	\file       limpieza_list.php
 *		\ingroup    persona2
 *		\brief      List page for limpieza
 */
...
require_once __DIR__.'/class/limpieza.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('persona2', 'limpieza', 'write');
	$permissiontodelete = $user->hasRight('persona2', 'limpieza', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("persona2")) {
	accessforbidden('Module persona2 not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Limpieza';
	$objectlabel = 'Limpieza';
	$uploaddir = $conf->persona2->dir_output;
...
$title = $langs->trans("Limpiezas");
//$help_url = "EN:Module_Limpieza|FR:Module_Limpieza_FR|ES:Módulo_Limpieza";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/persona2/limpieza_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/persona2/limpieza_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendLimpiezaRef";
$modelmail = "limpieza";
$objecttmp = new Limpieza($db);
